...
template<class T>
inline void Tree<T>::Reflect(Node* current)  //Mirror ,change sides for tree ,if its a leef change sides
{
	if (current == NULL)
		return;
	else
...
inline void Tree<T>::BreadThscan(Node* current)
{
	...
	while (!qe.isEmpty())    //while the queue is not empty
	{
		Node* node = qe.front();
		cout << node->value << " ";
		qe.dequeue();
